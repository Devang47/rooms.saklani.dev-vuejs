...
import { useMessagesStore } from '@/stores/global'
...
export const addMessage = async ({ roomId, message }: { roomId: string; message: string }) => {
  const cryptedKey = CryptoJS.SHA512(roomId).toString(CryptoJS.enc.Hex)
  const cryptedDeviceDetails = CryptoJS.SHA256(navigator.userAgent).toString(CryptoJS.enc.Hex)
...
export const getRoomMessages = async (roomId: string, scrollToBottom: () => void) => {
  const cryptedKey = CryptoJS.SHA512(roomId).toString(CryptoJS.enc.Hex)
  ...
  onSnapshot(dataQuery, (snapshot) => {
    ...
    messagesStore.setRoomMessages(msgs)
